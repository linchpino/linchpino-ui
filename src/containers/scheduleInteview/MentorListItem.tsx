...
const MentorListItem: FC<MentorListItemProps> = (props) => {
    const {availableTimeFrom,availableTimeTo,title,onSelect} = props
    return (
        <div className="relative w-full rounded-md flex flex-col m-4 shadow-lg h-[310px] justify-between items-center px-2 py-4">
            ...
            <p className="text-[#4B4141] text-[12px]">From :{!empty(availableTimeFrom) ?  moment(availableTimeFrom).format('ddd, D MMMM YYYY, HH:mm') : ""}</p>
            <p className="text-[#4B4141] text-[12px]">To :{!empty(availableTimeTo) ?  moment(availableTimeTo).format('ddd, D MMMM YYYY, HH:mm') : ""}</p>
