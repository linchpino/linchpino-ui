...
import {BsPlus, BsTrashFill, BsPencilFill} from 'react-icons/bs'
...
const ProfileTimeSlot: React.FC<ProfileTimeSlotProps> = ({
    ...
    const [selectedStartEdit, setSelectedStartEdit] = useState<Date | null>(currentDate);
    const [selectedStartTimeEdit, setSelectedStartTimeEdit] = useState<Date | null>(currentDate);
    const [selectedEndEdit, setSelectedEndEdit] = useState<Date | null>(nextMonthDate);
    const [selectedEndTimeEdit, setSelectedEndTimeEdit] = useState<Date | null>(nextMonthDate);
    const [selectedDurationEdit, setSelectedDurationEdit] = useState<number>(15);
    const [selectedDayEdit, setSelectedDayEdit] = useState<string[]>([]);
    const [selectedIntervalEdit, setSelectedIntervalEdit] = useState("1");
                    ...
                    <div className="relative bg-white rounded-lg shadow-xl text-xs sm:text-sm md:text-base lg:text-sm">
                        <div className="absolute top-4 left-2 flex gap-1">
                            <button onClick={openEditModal}
                                    ...
                                    className="tooltip rounded-full bg-blue-500 text-white hover:bg-blue-600 w-7 h-7 flex items-center justify-center">
                                <BsPencilFill/>
                            </button>
                            <button onClick={() => setIsDeleteModalOpen(true)}
                                    data-tip={t("ProfileTimeSlot.deleteButton")}
                                    className="tooltip p-1 rounded-full bg-red-500 text-white hover:bg-red-600 w-7 h-7 flex items-center justify-center">
                                <BsTrashFill/>
                            </button>
                        </div>
                        <div
            ...
            {isModalEdit && (
                <dialog ref={modalRef} id="modal" className={`modal modal-open`}>
                    <div
                        ...
                        <form >
                            <button onClick={() => setIsModalEdit(false)} className="btn btn-sm btn-circle btn-ghost absolute right-2 top-2">✕</button>
                        </form>
                        <div
                            className="flex flex-col pb-3 lg:pb-6 items-center justify-center w-full rounded-md mt-2 mb-4 lg:mb-0 container p-3">
                            <h1 className="text-xl text-center text-[#000]">{t("Profile.AddScheduleModal.header")}</h1>
